  ...
  return (
    ...
    >
      <div className="flex flex-wrap items-center gap-2 pointer-events-auto min-w-0">
        <div className="flex-1 min-w-[10rem] w-full sm:w-auto flex items-center gap-2 rounded-xl bg-[#0a1020]/95 border border-slate-700/80 shadow-lg px-3 py-2 backdrop-blur-sm">
        ...
        <button
          ...
          title={labelsOn ? 'Hide asset names on the map' : 'Show asset names on the map'}
          aria-pressed={labelsOn}
          className={`h-9 px-3 rounded-lg border text-[11px] font-black tracking-wide flex items-center gap-1.5 shrink-0 shadow-lg transition ${
            labelsOn
              ? 'tams-az-labels-on bg-[#0891B2] border-[#0e7490] text-white hover:bg-[#0e7490]'
              : 'tams-az-float bg-white border-[#c9d6df] text-[#0f172a] hover:border-[#0891B2]'
          }`}
        >
          <Tag className="w-3.5 h-3.5 shrink-0" strokeWidth={2.4} />
          <span>Labels</span>
        </button>
        {showOpsReopen && onOpenOpsPanel && (
          <button
            ...
            className="tams-az-float flex h-9 shrink-0 items-center gap-1.5 rounded-lg border border-slate-600 bg-slate-950 px-2.5 text-slate-200 shadow-lg transition hover:border-cyan-500/50 hover:bg-slate-800 hover:text-white"
          >
            <ChevronLeft className="h-3.5 w-3.5 shrink-0" strokeWidth={2.5} />
            <span className="text-[11px] font-black tracking-wide">Ops</span>
          </button>
        )}
        {controlRailCollapsed && onExpandMapTools && (
          <button
            ...
            className="tams-az-float flex h-9 shrink-0 items-center gap-1.5 rounded-lg border border-slate-600 bg-slate-950 px-2.5 text-slate-200 shadow-lg transition hover:border-blue-400/50 hover:bg-slate-800 hover:text-white"
          >
            <Maximize2 className="h-3.5 w-3.5 shrink-0" />
            <span className="text-[11px] font-black tracking-wide">Tools</span>
          </button>
        )}
      </div>
